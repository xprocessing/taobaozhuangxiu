...
/**
 ...
 2.根元素类定义包含：class="tb-module tshop-um tshop-um-neo-accordiontb"（class属性可以添加您需要的类选择器定义）
...
<div class="tb-module tshop-um tshop-um-neo-accordiontb">
...
<?
	 ...
	 $items = $itemManager->queryByIds($ids,"hotsell");}
		...
		foreach($items as $item ){
		 ...
		 echo '<div class="neo-panel"><a href="'.$uriManager-> detailURI($item).'" target="_blank"><img src="'.$item-> getPicUrl(220).'"/></a>';
			}
			else
			{
				 echo  '<div class="neo-trigger">'.$imore.'<a href="'.$uriManager-> detailURI($item).'" target="blank">'.$item->title.'</a></div>';
				 echo '<div class="neo-panel" style="display:none;"><a href="'.$uriManager-> detailURI($item).'" target="_blank"><img src="'.$item-> getPicUrl(220).'"/></a>';
				}
